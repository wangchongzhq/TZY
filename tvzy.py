...
def write_output_file(category_channels, debug_mode=False):
    ...
    if debug_mode:
        print(f"澶勭悊 {len(category_channels)} 涓被鍒?)
    ...
    for category in CATEGORY_ORDER:
        ...
        for channel_name, lines in category_channels[category].items():
            if debug_mode:
                print(f"  澶勭悊棰戦亾: {channel_name}, {len(lines)} 涓嚎璺?)
    ...
    if debug_mode:
        print(f"鎬诲叡鐢熸垚 {len(output_lines)} 琛屽唴瀹?)
        print(f"鍐欏叆鏂囦欢: {OUTPUT_FILE}")
...
def main():
    ...
    if debug_mode:
        print(f"寮€濮嬪鐞?{len(GITHUB_SOURCES)} 涓暟鎹簮")
    ...
    with concurrent.futures.ThreadPoolExecutor(max_workers=MAX_WORKERS) as executor:
        ...
        for future in concurrent.futures.as_completed(future_to_source):
            try:
                ...
                if debug_mode:
                    print(f"鑾峰彇鍒?{len(channels)} 涓閬?)
                all_channels.extend(channels)
            except Exception as e:
                if debug_mode:
                    print(f"澶勭悊鏁版嵁婧愭椂鍑洪敊: {e}")
