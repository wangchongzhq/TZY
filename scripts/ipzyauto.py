...
def fetch_lines_with_retry(url: str, max_retries=3, timeout=None):

    """带重试机制的下载函数，利用现有的网络模块功能"""

    # 如果没有传入timeout，则使用默认值
    ...
    for attempt in range(1, max_retries + 1):
        ...
        try:
            ...
            print(f"  尝试 {attempt}/{max_retries}: 直接请求")
            ...
            if response.status_code == 200:
                ...
                print(f"  成功获取，耗时 {elapsed_time:.2f}秒")
            ...
            print(f"  直接请求失败: {type(e).__name__}: {str(e)}")

        

        if attempt < max_retries:

            print(f"  {attempt}/{max_retries} 失败，等待1秒后重试...")

            time.sleep(1)

    

    # 如果直接请求失败，尝试使用network模块的fetch_content函数

    try:

        from core.network import fetch_content

        print("  回退到network模块的fetch_content函数")

        content = fetch_content(url, retries=max_retries, timeout=timeout)

        if content:

            print(f"  使用network模块成功获取内容")

            return content.splitlines()

    except Exception as e:

        print(f"  network模块请求失败: {type(e).__name__}: {str(e)}")

    

    print(f"  所有尝试都失败，跳过该URL")
...
def create_txt_file(all_channels, filename="ipzyauto.txt", speed_results=None):
    ...
    content.append("分组: 4K频道,央视频道,卫视频道,北京频道,山东频道,港澳频道,电影频道,儿童频道,iHOT频道,综合频道,体育频道,剧场频道,其他")
    ...
    for channel, urls in list(all_channels.items()):
        ...
        if not channel.startswith('CCTV') and urls:
            ...
            if other_written % 50 == 0:
                ...
                print(f"      已写入 {other_written} 个其他...")



    print(f"  其他写入完成，共 {other_written} 个频道")
...
def load_sources_from_json():
    ...
    try:
        ...
        # 直接使用UNIFIED_SOURCES，该文件会由update_sources.py自动更新

        print(f"从统一播放源加载了 {len(UNIFIED_SOURCES)} 个启用的直播源")
...
def main():
    ...
    for url in urls:
        ...
        lines = fetch_lines_with_retry(url, max_retries=max_retries, timeout=timeout)
