...
class CodeValidator:
    ...
    def log_warning(self, message: str, file_path: str = "", line: int = 0):
        """记录警告"""
        if file_path and line > 0:
            self.warnings.append(f"⚠️  {file_path}:{line} - {message}")
        else:
    ...
    def check_dependencies(self) -> bool:
        ...
        if req_file.exists():
            try:
                ...
                # 检查关键依赖
                required_deps = ['requests', 'tkinter', 'threading']
                missing_deps = []
        ...
        for bat_file in batch_files:
            try:
                ...
                if re.search(r'[^\x00-\x7F]', content):
                    if 'chcp 936' not in content:
                        self.log_warning(f"包含中文字符但未设置GBK编码", str(bat_file))
                        
            except UnicodeDecodeError:
                self.log_error(f"批处理文件编码错误", str(bat_file))
                batch_ok = False
            except Exception as e:
                self.log_error(f"批处理文件检查失败: {e}", str(bat_file))
                batch_ok = False
