...
CATEGORY_RULES = {
    ...
    "境外台": [
        r'鍑ゅ嚢', r'TVB', r'缈＄繝', r'鏄庣彔', r'鏈腐', r'鍥介檯', r'婢宠', r'婢抽棬',
...
def normalize_channel_name(name):
    ...
    replacements = {
        ...
        r'CCTV7': 'CCTV-7',
        r'CCTV-8\s*鐢佃鍓?': 'CCTV-8',
        r'CCTV8': 'CCTV-8',
        r'CCTV-9\s*绾綍': 'CCTV-9',
        r'CCTV9': 'CCTV-9',
                ...
                if is_hd_channel(current_channel):
                    ...
                    if channel_name not in channels:
                        channels[channel_name] = {
                            ...
                            'group': current_channel.get('group', '榛樿鍒嗙粍'),
...
def write_output_file(channels_by_category):
    ...
    with open('ipzy_channels.txt', 'w', encoding='utf-8') as f:
        f.write(f"# 涓浗澧冨唴鐢佃鐩存挱绾胯矾 (浠呴檺1080p楂樻竻浠ヤ笂)\n")
        ...
        category_order = ["澶", "鍗", "娓境鍙?, "褰辫鍓?, "4K", "闊充箰", "鍏朵粬"]
        ...
        for category in category_order:
            if category in channels_by_category and channels_by_category[category]:
                ...
                f.write(f"# 鍏?{len(sorted_channels)} 涓閬擄紝{category_url_count} 鏉＄嚎璺?(骞冲潎{avg_urls:.1f}鏉?棰戦亾)\n\n")
        
        f.write("# 鑷姩鐢熸垚 - 姣忔棩鍖椾含鏃堕棿涓?鐐规洿鏂癨n")
        f.write("# 浠呬繚鐣?080p楂樻竻鍙婁互涓婃竻鏅板害绾胯矾\n")
        f.write("# 姣忎釜棰戦亾鑷冲皯10鏉＄嚎璺紝鏈€澶?0鏉＄嚎璺痋n")
...
def main():
    ...
    print(f"鍏卞彂鐜?{len(SOURCES)} 涓暟鎹簮")
    
    for i, source in enumerate(SOURCES, 1):
        try:
            # SOURCES_WITH_NAMES鏄竴涓厓缁勫垪琛細(name, url)
            ...
            if content:
                ...
                print(f"  鉁?瑙ｆ瀽鎴愬姛锛屽彂鐜?{len(channels)} 涓閬?)
    ...
    try:
        ...
        print(f"  鉁?鍚堝苟瀹屾垚锛屽叡 {len(merged_channels)} 涓閬?)
    except Exception as e:
